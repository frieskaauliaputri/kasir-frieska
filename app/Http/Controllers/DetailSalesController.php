...
class DetailSalesController extends Controller
{
    ...
    public function exportexcel()
    {
        if (Auth::user()->role == 'employee') {
            $sales = saless::with(['customer', 'user', 'detail_sales.product'])->get();
    
            return (new FastExcel($sales))->download('Penjualan.xlsx', function ($sale) {
                return [
                    'ID Penjualan'     => $sale->id,
                    'Tanggal'          => $sale->sale_date,
                    'Nama Customer'    => $sale->customer->name ?? 'NON-MEMBER',
                    'No HP'            => $sale->customer->no_hp ?? '-',
                    'Total Harga'      => $sale->total_price,
                    'Total Produk'     => $sale->detail_sales->sum('amount'),
                    'Dibuat Oleh'      => $sale->user->name ?? '-',
                    'Tanggal Input'    => $sale->created_at->format('d-m-Y H:i'),
                ];
            });
        }
